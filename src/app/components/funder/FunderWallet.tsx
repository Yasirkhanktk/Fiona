...
import { Wallet, ArrowUpRight, ArrowDownRight, Clock, DollarSign, TrendingUp, Globe, ArrowRightLeft, PiggyBank, Send, Download } from 'lucide-react';
...
export function FunderWallet() {
  ...
  const transactions = [
    ...
    {
      type: 'outbound',
      description: 'Capital deployment - CR-2024-004',
      amount: 850000000,
      currency: 'COP',
      date: '2024-02-15',
      status: 'completed',
    ...
    {
      type: 'inbound',
      description: 'Principal repayment - CR-2024-005',
      amount: 125000,
      currency: 'USD',
      date: '2024-02-12',
      status: 'completed',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-slate-200">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Multi-Currency Capital Management</h2>
          <p className="text-sm text-slate-600">Manage capital across global markets and currencies</p>
        </div>
      </div>
      ...
      >
        <Card className="bg-gradient-to-br from-amber-600 via-orange-600 to-red-600 text-white overflow-hidden relative">
          ...
          <CardContent className="pt-8 pb-8 relative">
            <div className="flex items-center justify-between mb-6">
              <div>
                <p className="text-amber-100 text-sm mb-2 flex items-center gap-2">
                  <PiggyBank className="w-4 h-4" />
                  Total Capital Under Management (USD Equivalent)
                </p>
                ...
                <div className="flex items-center gap-2 text-amber-100">
                  ...
                  <span className="text-sm font-semibold">+3.5% this month</span>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <Button className="bg-white hover:bg-slate-100 text-amber-600 font-semibold">
            ...
            <div className="grid grid-cols-4 gap-4">
              {wallets.map((wallet) => (
                <div key={wallet.currency} className="bg-white/10 backdrop-blur rounded-lg p-3 border border-white/20">
                  ...
                  <div className={`text-xs font-semibold ${wallet.change24h >= 0 ? 'text-amber-200' : 'text-red-300'}`}>
              ...
              <CardContent className="pt-6">
                <div className={`w-full h-32 rounded-xl bg-gradient-to-br ${wallet.gradient} p-4 mb-4 shadow-lg`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="text-white/80 text-sm font-medium">Capital Available</div>
              ...
              <div className="space-y-3">
                {transactions.filter(t => t.type === 'outbound').map((transaction, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="font-bold text-xl mb-1 text-amber-600">
                        -{wallets.find(w => w.currency === transaction.currency)?.symbol}
                        {transaction.amount.toLocaleString()}
                      </p>
                      <Badge
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="border-2 border-amber-100">
          ...
          <CardContent className="space-y-3">
            <Button className="w-full justify-start gap-3 h-12 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700" size="lg">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-white border-2 border-slate-200">
                <p className="text-sm text-slate-600 mb-1">Primary Custody Account</p>
                <p className="font-mono font-bold text-lg">CUST-FND-2024-001</p>
              </div>
              <div className="p-4 rounded-xl bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200">
                <p className="text-sm text-amber-900 mb-2 font-semibold">Multi-Currency Support</p>
                <div className="flex gap-2 flex-wrap">
                  {wallets.map(wallet => (
                    <Badge key={wallet.currency} className="bg-white text-amber-700 border border-amber-300">
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3 text-2xl">
              ...
              Manage {currencyToManage?.currency} Capital
            </DialogTitle>
          </DialogHeader>
          ...
          {currencyToManage && (
            <div className="space-y-6">
              ...
              <div className={`p-6 rounded-xl bg-gradient-to-br ${currencyToManage.gradient} text-white shadow-lg`}>
                <p className="text-white/80 text-sm mb-2">Capital Available</p>
                ...
                <Card>
                  <CardContent className="pt-4">
                    ...
                    <p className="text-xl font-bold text-amber-600">
                      {currencyToManage.symbol}{(currencyToManage.balance * 0.35).toLocaleString()}
                    </p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-4">
                    <p className="text-sm text-slate-600 mb-1">Returns</p>
                    <p className="text-xl font-bold text-green-600">
                      +{currencyToManage.symbol}{(currencyToManage.balance * 0.12).toLocaleString()}
                    </p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="pt-4">
                    ...
                    <p className="text-xl font-bold text-blue-600">
                      {currencyToManage.symbol}{(currencyToManage.balance * 0.08).toLocaleString()}
                    </p>
                  </CardContent>
                </Card>
              </div>
              ...
              {/* Actions */}
              <div>
                <h3 className="font-semibold text-lg mb-3">Capital Operations</h3>
                <div className="grid grid-cols-2 gap-3">
                  <Button className="h-16 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700">
                    <Send className="w-5 h-5 mr-2" />
                    Deploy {currencyToManage.currency}
                  </Button>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {wallets
                    ...
                    .map(wallet => {
                      ...
                      return (
                        <Card key={wallet.currency} className="border-2 hover:border-amber-300 transition-colors">
